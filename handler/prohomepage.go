...
//Profile image must have a aspect-ratio of 1:1
...
route.Get(func() {
    ...
    jobs := store.GetEntrepriseJobs(conn, route.User.Id)
...
route.Put(nil, func() {
    ...
    defer conn.Close()
    interviews := interview.Interviews(conn, route.User.Id)
    interviewData := InterviewData{Interview: interviews, InterviewType: store.GetInterviewType(conn)}
            ...
            {{range .Interview}}
                <div class="interview-card">
                    ...
                    <dialog class="interview-modal">
                        ...
                        <form hx-put="/interview/{{.Id}}" hx-vals='{"name": "{{.UserName}}", "addr": "{{.Addr}}"}'  hx-ext="json-enc" hx-target="#interview-{{.Id}}" hx-swap="outerHTML">
                            <div style="display: flex;justify-content: space-between;gap: .5rem;">
                                <div>
                                    <b class="input-label">Date</b>
                                    <input type="datetime-local" name="interview_date" class="interview-input" value="{{.Date}}" />
                                </div>
                                <div>
                            ...
                            <div class="interviewModal-location hidden" style="position: relative;">
                                ...
                                <input type="text" name="addr" autocomplte="off" class="interview-input" oninput="onFetchAddr(this)"/>
