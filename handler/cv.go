...
var CurriculumHandler = func(res http.ResponseWriter, req *http.Request){
    ...
    route.Get(func() {
        ...
        defer conn.Close()
        if format == "pdf"{
            cv.Get(conn, store.EncryptCurriculumId(route.User.Id))
        ...
        if err := cv.Get(conn, cv.UserId); err != nil{
            log.Println(err)
            return
        }
        ...
        application.UpdateStatus(conn, "Vue")
    ...
    route.Put(nil, func() {
        ...
        conn := store.GetDBPoolConn()
        defer conn.Close()
        if err := application.UpdateStatus(conn, route.UrlEncoded["status"]); err != nil{
            route.Notification("error", "error requete impossible")
            return
        }
        updatedApplication := ApplicationUpdate{InterviewType: store.GetInterviewType(conn), Application: application}
