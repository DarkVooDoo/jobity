...
var ProInterviewHandler = func(res http.ResponseWriter, req *http.Request){
    route, err := NewRoute(res, req)
    if err != nil{
        return
    }
    
    interview := store.Interview{Id: route.Request.PathValue("id")}
    route.Get(func() {
        route.Render(nil, "route/protemplate.html", "route/interview.html")
    })

    route.Post(&interview, func() {
        conn := store.GetDBPoolConn()
        interview.UserId = store.DecryptCurriculumId(interview.UserId)
        defer conn.Close()
        if err := interview.Create(conn); err != nil{
            route.Notification("error", "on a pas pu creer l'interview")
            return
        }
        application := store.JobApplication{Id: route.Request.PathValue("id")}
        if err := application.Delete(conn); err != nil{
            route.Notification("error", "on a pas pu supprimer l'application")
            return
        }
        route.Notification("success", "interview planifié")
    })

    route.Put(&interview, func() {
        conn := store.GetDBPoolConn()
        defer conn.Close()
        if err := interview.Modify(conn); err != nil{
            route.Notification("error", "error dans la requete")
            return
        }
        route.Notification("success", "interview modifié")
        temp, err := template.New("update-interview").Parse(`
             <div id="interview-{{.Id}}">
                  <div class="interview-card-info">
                      <img src="/static/netflix.webp" class="interview-photo" />
                      <div style="position: relative; width: 100%">
                          <img src="/static/{{if eq .Type "En personne"}}talk{{else}}{{.Type}}{{end}}.svg" class="interview-type" id="icon-{{.Id}}"/>
                          ...
                          {{end}}
                          {{if eq .Type "Video"}}<a href="/interview/{{.Id}}">Commencer l'interview</a>{{end}}
                      </div>
                  </div>
        ...
        if err := temp.Execute(route.Response, interview); err != nil{
